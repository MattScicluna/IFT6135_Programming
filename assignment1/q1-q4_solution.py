...
class BassetDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        * When in doubt, look at the output of __getitem__ !
        ...
        # WRITE CODE HERE
        out_seq = torch.from_numpy(self.inputs[idx]).float()  # change to float32
        out_seq = out_seq.permute(1,2,0)  # (C, H, W) = (1, 600, 4)

        output['sequence'] = out_seq

        out_tgt = torch.from_numpy(self.outputs[idx]).float()  # change to float32
        output['target'] = out_tgt
    ...
    def get_seq_len(self):
        ...
        """
        # WRITE CODE HERE
        return self[0]['sequence'].shape[1]
...
class Basset(nn.Module):
    ...
    def forward(self, x):
        ...
        # WRITE CODE HERE
        x = self.maxpool1(F.relu(self.bn1(self.conv1(x))))           # batch_size x 300 x 200 x 1
        x = self.maxpool2(F.relu(self.bn2(self.conv2(x))))           # batch_size x 200 x 50 x 1
        x = self.maxpool3(F.relu(self.bn3(self.conv3(x))))           # batch_size x 200 x 13 x 1
        x = x.view(-1, 13*200)

        x = F.dropout(F.relu(self.bn4(self.fc1(x))), p=self.dropout, training=self.training)  # batch_size x 1000
        x = F.dropout(F.relu(self.bn5(self.fc2(x))), p=self.dropout, training=self.training)  # batch_size x 1000
...
def compute_fpr_tpr(y_true, y_pred):
    ...
    # WRITE CODE HERE
    output['tpr'] = (y_pred*y_true).sum()/y_true.sum()
    output['fpr'] = (y_pred*(1-y_true)).sum()/(1-y_true).sum()
...
def compute_fpr_tpr_dumb_model():
    ...
    # WRITE CODE HERE
    y_model = np.random.uniform(0, 1, 1000)
    y_true = np.random.choice(a=[0,1], size=1000, replace=True)

    for k in np.arange(0, 1, 0.05):
        y_pred = y_model >= k
        _o = compute_fpr_tpr(y_true, y_pred)
        output['tpr_list'].append(_o['tpr'])
        output['fpr_list'].append(_o['fpr'])
...
def compute_fpr_tpr_smart_model():
    ...
    y_model = np.zeros(1000)
    y_model[y_true == 1] = np.random.uniform(0.4, 1, (y_true == 1).sum())
    y_model[y_true == 0] = np.random.uniform(0, 0.6, (y_true == 0).sum())
...
def compute_auc_both_models():
    ...
    y_model = np.random.uniform(0, 1, 1000)
    y_true = np.random.choice(a=[0,1], size=1000, replace=True)
    out = compute_auc(y_true, y_model)
    output['auc_dumb_model'] = out['auc']
    ...
    y_model[y_true == 0] = np.random.uniform(0, 0.6, (y_true == 0).sum())
    out = compute_auc(y_true, y_model)
    output['auc_smart_model'] = out['auc']
    ...
    * You should collect all the targets and model outputs and then compute AUC at the end
    ...
    for i, out in enumerate(dataloader):
        ...
        y_true = np.concatenate([y_true,
                                 trgs.cpu().detach().numpy().flatten()])
        y_model = np.concatenate([y_model,
                                  predictions.cpu().detach().numpy().flatten()])

        if i == 100:
            break

    output = compute_auc(y_true, y_model)
    return output
...
def compute_auc(y_true, y_model):
    ...
    for k in np.arange(0, 1, 0.05):
        y_pred = y_model >= k
        out_fpr_tpr = compute_fpr_tpr(y_true, y_pred)
        tpr.append(out_fpr_tpr['tpr'])
        fpr.append(out_fpr_tpr['fpr'])

    right_reimann_sum = (np.diff(np.array(fpr)[::-1])*
                         np.array(tpr)[::-1][1:]).sum()  # right Reimann sums

    left_reimann_sum = (np.concatenate([np.diff(np.array(fpr)[::-1]), np.array([0])])*
                        np.concatenate([np.array([0]), np.array(tpr)[::-1][1:]])).sum()  # left Reimann sums

    output['auc'] = (right_reimann_sum + left_reimann_sum) / 2
    ...
    #widths = np.diff(np.array(fpr)[::-1])
    #heights = np.array(tpr)[::-1][1:]

    #for i in range(len(widths)):
    #    ax.add_patch(matplotlib.patches.Rectangle((np.array(fpr)[::-1][i], 0),
    #                                              width=widths[i],
    #                                              height=heights[i],
    #                                              angle=0.0,
    #                                              alpha=0.3))
...
def train_loop(model, train_dataloader, device, optimizer, criterion):
    ...
    model.train()
    for i, out in enumerate(train_dataloader):
        ...
        train_loss = criterion(predictions, trgs)

        train_loss.backward()
        optimizer.step()

        output['total_loss'] += train_loss.detach()

        if (i % 50 == 0) & (i > 0):
            n += 1

            predictions = 1/ (1 + np.exp(-predictions.flatten().cpu().detach().numpy()))
            output['total_score'] += compute_auc(trgs.flatten().cpu().detach().numpy(),
                                                 predictions)['auc']
            ...
            print('[{}/{}]: loss={:.5f} score={:.5f}'.format(i,
                                                             len(train_dataloader),
                                                             output['total_loss']/i,
                                                             output['total_score']/n))
...
def valid_loop(model, valid_dataloader, device, optimizer, criterion):
    ...
    for i, out in enumerate(valid_dataloader):
        seqs = out['sequence'].to(device)
        trgs = out['target'].to(device)

        predictions = model(seqs)
        output['total_loss'] += criterion(predictions, trgs).detach()
